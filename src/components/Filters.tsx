...
export default function Filters({ productList, setList, list }: { productList: Product[], setList: (newState: any) => void, list: Product[] }) {
    ...
    const filterCategory = (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        if (e.target.value === 'Category') {
            ...
            const listToFilter = !isFiltered.includes('Category') && isFiltered.length > 0 ? list : productList

            setList(listToFilter.filter(product => product.category === e.target.value))
            !isFiltered.includes('Category') && setFilter(prev => [...prev, 'Category'])
        }
    }
    ...
    const filterType = (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        if (e.target.value === 'Product type') {
            ...
            const listToFilter = !isFiltered.includes('Type') && isFiltered.length > 0 ? list : productList
            setList(listToFilter.filter(product => product.product_type <= e.target.value && product.product_type >= e.target.value + 1))
